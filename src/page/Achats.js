...
import AchatsData from '../data.js/AchatsData';
    ...
    />
      ...
      {AchatsData.map((achat , index) =>  (
       <div key={index} className='grid gap-2 grid-cols-4 md:grid-cols-7 text-center place-content-center bg-gray-400/30  w-[98%] my-2 py-3 rounded-xl justify-center'>
         <h1>{achat.id_produit}</h1>
         <h1>{achat.id_fournisseur}</h1>
         <h1 className='hidden md:flex md:justify-center'>{achat.dateAchat.toLocaleDateString()}</h1>
